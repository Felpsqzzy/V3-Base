...
function publicRow(row) {
  return { id:row.id,namespace:row.namespace,recordId:row.record_id,payload:row.payload,deleted:row.deleted,version:Number(row.version),updatedBy:row.updated_by,updatedAt:row.updated_at,createdAt:row.created_at };
}
  ...
  try {
    if (method === 'GET') {
      ...
      const rows = await withTransaction(async client => {
        ...
        let sql = `SELECT id, namespace, record_id, payload, deleted, version, updated_by, updated_at, created_at
                     FROM app.sync_registro WHERE namespace = $1`;
      ...
      const row = await withTransaction(async client => {
        const currentResult = await client.query(
          `SELECT id, namespace, record_id, payload, deleted, version, updated_by, updated_at, created_at
             FROM app.sync_registro WHERE namespace = $1 AND record_id = $2 FOR UPDATE`,
          [namespace, recordId]
        );
        ...
        if (!current) {
          ...
          const insert = await client.query(
            `INSERT INTO app.sync_registro (namespace, record_id, payload, deleted, version, updated_by, updated_at)
        ...
        const update = await client.query(
          `UPDATE app.sync_registro SET payload=$3::jsonb, deleted=$4, version=version+1, updated_by=$5, updated_at=now()
